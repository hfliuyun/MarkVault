...
- 已优化移动端文章侧边栏：手机宽度下默认优先展示正文，通过文章导航抽屉查看系列目录和正文 TOC，点击 TOC 后会定位标题并收起抽屉。
- 已实现 Markdown HTML 安全清洗：后端 Markdown 渲染后通过白名单保留常用文章标签和必要属性，移除脚本标签、事件属性和危险协议，降低前端 `v-html` 注入风险。
